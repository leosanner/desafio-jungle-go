...
// EventBus. HandleInbound writes inbox + domain in one unit of work.
// PendingResumer claims PENDING / PENDING_REFERENCE rows and re-applies them.
// Actor is the authenticated caller derived from the IdP (HTTP) or queue-gated
// providerId (SQS).
package app
