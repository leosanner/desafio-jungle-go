...
| ELIM-03 | No floating-point money arithmetic | — | |
...
| STK-05 | DB library, `Money` mapping and cross-repository transaction documented | — | [ADR 0003](../adr/0003-database-access-and-migrations.md) (`pgx/v5` only) | Money mapping and unit of work still TBD |
...
| ID | Requirement | Status | Evidence | Notes |
| --- | --- | --- | --- | --- |
| GAR-01 | Money without floats in parsing, arithmetic, serialization and persistence | — | | |
...
| ID | Requirement | Status | Evidence | Notes |
| --- | --- | --- | --- | --- |
| DOM-01 | Encapsulated state, validating constructors, explicit transitions | — | | |
| DOM-02 | Separate creation and rehydration | — | | |
| DOM-03 | Uninitialized/invalid values rejected | — | | |
| DOM-04 | Classifiable errors (`errors.Is`/`As`), no `panic` for business rules | — | | |
| DOM-05 | I/O takes `context.Context` | — | | |
| MON-01 | Immutable `Money`: parse from string, zero, add, subtract, negate, compare, serialize | — | | |
| MON-02 | Contract `{"amount":"25.00","currency":"BRL"}`, scale 2, ISO 4217 | — | | |
| MON-03 | Rejects empty, NaN, Infinity, scientific notation, excess scale, negative external input | — | | |
| MON-04 | No silent rounding; normalization before hashing documented | — | | |
| MON-05 | Incompatible currencies rejected (with tests) | — | | |
| MON-06 | Overflow handled (if `int64`) | — | | |
| MON-07 | Exact persistence of amount and currency; representation and limits documented | — | | |
| WAL-01 | Identity, player, currency, balance, version, timestamps | — | | |
| WAL-02 | One wallet per `(playerId, currency)` | — | | |
| WAL-03 | Debit keeps balance ≥ 0; currency matches | — | | |
| WAL-04 | Financial change with ledger entry in the same commit | — | | |
| WAL-05 | Initial version 1, increments only on balance change | — | | |
| WAL-06 | Concurrency strategy documented | — | | |
| WTX-01 | External transaction fields persisted (ids, provider, key, hash, round, game, reference, result) | — | | |
| WTX-02 | Validated state machine; immutable terminal states; documented | — | | |
| WTX-03 | Replay returns persisted result without re-applying | — | | |
| WTX-04 | `PENDING` durably resumable by another instance | — | | |
| WTX-05 | `OPENING` rejected via HTTP/SQS; schema distinguishes internal/external; no duplicate opening credit | — | | |
| WTX-06 | Transient vs permanent failure distinction documented | — | | |
| LED-01 | Entry fields and `balanceAfter = balanceBefore ± money` validation | — | | |
| LED-02 | `(walletId, transactionId)` uniqueness and update/delete protection in the database | — | | |
| LED-03 | `LOSS` and rejections produce no entry | — | | |
...
| ID | Requirement | Status | Evidence | Notes |
| --- | --- | --- | --- | --- |
| OPS-01 | `BET`: debit, positive amount, sufficient funds | — | | |
| OPS-02 | `WIN`: credit, positive amount, optional reference to a bet in the same round | — | | |
| OPS-03 | `LOSS`: `"0.00"`, no ledger, no version bump, emits `WagerTransactionProcessed` | — | | |
| OPS-04 | `REFUND`: fully returns a processed `BET` | — | | |
| OPS-05 | `ROLLBACK`: fully reverses a processed `BET`/`WIN`/`REFUND` | — | | |
| OPS-06 | Mandatory reference resolved by `(providerId, referenceExternalTransactionId)` | — | | |
| OPS-07 | Reference matches provider, player, wallet, currency, round and amount | — | | |
| OPS-08 | No two successful reversals of the same kind; REFUND/ROLLBACK combinations documented | — | | |
| OPS-09 | Reversal without funds rejected and auditable with its own `failureCode` | — | | |
...
| OPS-12 | Behavior with a pending or unsuccessful reference documented | — | | |
| OPS-13 | Stable, documented `failureCode`, correctable × definitive | — | | |
...
| HTTP-08 | Replay with `idempotentReplay: true` and original balance; conflict on different payload | — | | |
| HTTP-09 | Same `(providerId, externalTransactionId)` cannot be re-applied under another key | — | | |
...
| OBX-05 | Events `WagerTransactionProcessed`, `Rejected`, `WalletBalanceChanged`, `PendingReference` with concrete types | — | | |
| OBX-06 | Envelope `eventId`, `eventType`, `aggregateId`, `correlationId`, `causationId?`, `occurredAt`, `version`, `data` | — | | |
| OBX-07 | Complete `WalletBalanceChanged` payload | — | | |
| OBX-08 | Type/version set by constructor; UTC RFC 3339; money as strings; immutable snapshot | — | | |
...
| ID | Requirement | Status | Evidence | Notes |
| --- | --- | --- | --- | --- |
| TST-01 | Unit: Money (parsing, scale, limits, invalid input, currencies) | — | | |
| TST-02 | Unit: wallet invariants and state transitions | — | | |
| TST-03 | Unit: rules for the 5 kinds, zero policy, internal opening, payload conflict | — | | |
...
| TST-18 | `go test -race` on applicable tests | ✅ | `go test -race ./...` on unit packages; [ADR 0005](../adr/0005-test-strategy-initial.md) | Domain/concurrency suites not written yet |
...
| ENT-05 | ARCHITECTURE.md with decisions, limitations, interpretations and unfinished work | 🚧 | [ARCHITECTURE.md](../../ARCHITECTURE.md); [docs/adr/](../adr/) | Phase 1 ADRs 0001–0005; Money, locks, idempotency, auth model still TBD |
...
| ENT-08 | `gofmt`-formatted code and reproducible dependencies | 🚧 | [ADR 0005](../adr/0005-test-strategy-initial.md); `gofmt -l .` in README | Little application code in the docs pass |
